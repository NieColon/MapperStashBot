...
const tools = new DiscordTools('hidden');
exports.run = (client, message, args, tools) => { // This is the same handler we have been using in the previous episodes.
  ...
  if (!args) return tools.embed(message.channel, 'Please input some text'); // This is from the `Global Functions` episode. - This checks to see if args[0] is defined, if it's not it runs the return statement.
...
exports.settings = {
    name: `auto`,
    aliases: [`auto`, `complete`, `autocomplete`, `autouser`]
}